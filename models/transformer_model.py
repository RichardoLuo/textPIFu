...
class TransformerTextureAwareModel():
    ...
    def feed_data(self, data):
        ...
        b = self.image.size(0)
        for idx in range(b):
            image_feat = self.get_imagefeatures([sampled_indices[idx:idx + 1] for sampled_indices in self.input_indices],
                                             self.texture_mask[idx:idx + 1])
            self.img_feats.append(image_feat)

    def get_imagefeatures(self, indices_list, texture_mask):
        quant = self.img_quantizer.get_codebook_entry(
            indices_list, texture_mask,
            (indices_list[0].size(0), self.shape[0], self.shape[1],
             self.opt["img_z_channels"]))
